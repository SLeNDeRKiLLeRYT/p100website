...
export default function AdminPanelClient() {
  // The ?key= gate is enforced server-side in page.tsx before this renders.
  ...
  const fetchStorageArtworks = async () => {
    ...
    try {
      ...
      const items: StorageItem[] = (data || []).map((file: any) => {
      ...
      const artistMap: Record<string, string | null> = {};
      items.forEach(item => {
        const artwork = artworksData?.find((a: any) => a.artwork_url === item.publicUrl);
  ...
  const fetchStorageItems = async (bucket: string) => {
    ...
    try {
        ...
        const listItemsRecursively = async (pathPrefix = '') => {
            ...
            const files = data.filter((item: any) => item.id !== null);
            const folders = data.filter((item: any) => item.id === null);
            ...
            const folderPromises = folders.map((folder: any) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setAuthLoading(true);
    const loginResult = await adminLogin(password);
