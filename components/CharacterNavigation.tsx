...
export default function CharacterNavigation({ previous, next }: CharacterNavigationProps) {
    return (
        <>
            ...
            {previous && (
                <div className="flex fixed left-1 md:left-4 top-1/2 -translate-y-1/2 z-50 flex-col items-center">
                    <Link
                        href={`/${previous.type === 'killer' ? 'killers' : 'survivors'}/${previous.id}`}
                        className="relative flex items-center justify-center w-10 h-16 md:w-20 md:h-32 bg-black/80 hover:bg-black/60 border-2 border-red-600 hover:border-red-400 rounded-xl transition-all duration-200 hover:scale-105 shadow-lg backdrop-blur-sm group overflow-hidden"
                    ...
                    >
                        ...
                        <div className="relative z-10 flex items-center justify-center">
                            <ChevronLeft className="h-6 w-6 md:h-10 md:w-10 text-red-400 group-hover:scale-110 transition-transform drop-shadow-lg" />
                        </div>
                    </Link>
                    
                    {/* Text content underneath button - hidden on mobile */}
                    <div className="hidden md:block text-center mt-2 px-1">
            ...
            {next && (
                <div className="flex fixed right-1 md:right-4 top-1/2 -translate-y-1/2 z-50 flex-col items-center">
                    <Link
                    ...
                    >
                        ...
                        {/* Content - Only arrow icon */}
                        <div className="relative z-10 flex items-center justify-center">
                            <ChevronRight className="h-6 w-6 md:h-10 md:w-10 text-red-400 group-hover:scale-110 transition-transform drop-shadow-lg" />
                        </div>
                    </Link>
