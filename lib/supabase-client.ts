...
// createAdminClient moved to lib/supabase-admin.ts (server-only).
// It must NEVER live in this module: this file is imported by client components.
...
export const sanitizeInput = (input: string): string => {
  ...
  return input
    ...
    // Remove any remaining HTML tags while preserving things like "<3" (no closing '>')
    .replace(/<[^>]*>/g, '')
